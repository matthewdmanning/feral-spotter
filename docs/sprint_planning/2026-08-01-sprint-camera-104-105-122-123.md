# Sprint:camera — #104, #105, #122, #123 (2026-08-01)

Implements #105 + #104, #122, #123 — the build spec in
`docs/design/2026-07-31-sprint-camera-planning.md` (#121 umbrella →
#122, #105, #104, #123) on branch `issue-128-location-capture-timing`.
Code complete; **not yet run on a device/emulator** — see Verification
status below before merging.
...
**Rejected as no clean unit seam:** the `#94` `maximumDate` bound
(enforced by the native `RNDateTimePicker` itself;
`useDateTimePicker.handleConfirm` applies no bound of its own, so there's
no logic to assert against).
...
## #122 — Settings button removed

### What shipped

**Settings button removed** from Home header
(`src/screens/home/index.tsx`). The Settings tab itself is untouched.

No specific on-device risk was flagged for this item beyond the general
not-yet-run status — a header button removal has no permission/gesture/
native surface to check.

## #123 — Swipe-up-to-remove

### What shipped

**Swipe-up-to-remove** (`CameraThumb.tsx`): tap-X `Pressable` replaced
with a directional-locked `Gesture.Pan()` (`activeOffsetY`/`failOffsetX`,
same idiom as `useBoundingBoxFrame.ts`), driving `translateY`/opacity via
Reanimated, calling `onRemove` past a 60px upward-swipe threshold.
`accessibilityAction` ("Discard photo") kept as the non-gesture
screen-reader path.

### Tests

**Rejected as no clean unit seam:** swipe-gesture/accessibility-action
(`GestureDetector`/worklet — integration-level, not unit logic).

## Verification status

Gates run and passing (whole-sprint run, not isolated to any one item):
`tsc --noEmit` (both `tsconfig.json` and `tsconfig.server.json`), full
`jest` suite (23 suites / 82 tests), `expo lint` (0 errors, pre-existing
require()-in-tests warnings only).

**Not run on an emulator/device.** Before merging, at minimum check:

- Library picker end-to-end (#105/#104): picker actually launches, lazy
  permission prompt appears once, picked photos land in the pool,
  navigation to `/submission/create` happens.
- The new mid-capture `mediaLibrary` permission prompt (#105/#104; first
  shutter press behaves differently now than before this change).
- Date & Time warning row (#105/#104): single tap should open the picker
  modal directly (no intermediate state); visually it's an `AlertCircle` +
...
- Source-gate button disabling (#105/#104) was verified only against a
  mocked `source` value, not the live store end-to-end through actual
  button presses.
- Swipe-up-to-remove (#123) — the build spec itself flags a possible
  conflict with the Android system home/back edge gesture on a
  bottom-of-screen thumbnail strip; this needs a real on-device pass, not
  just a direction check.
