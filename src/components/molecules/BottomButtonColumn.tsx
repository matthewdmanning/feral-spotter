...
interface BottomButtonColumnProps {
  buttons: ColumnButton[]; visible: boolean; spacing?: number; paddingBottom?: number
}

export function BottomButtonColumn({
  buttons, visible, spacing = 12, paddingBottom = 16
        ...
        return (
          <View key={btn.key} style={i > 0 ? { marginTop: spacing } : undefined}>
            <Pressable
              ...
              style={[styles.btn, { backgroundColor: bgMap[variant] }, border, btn.disabled && styles.disabled]}
              accessibilityRole="button" accessibilityLabel={btn.accessibilityLabel ?? btn.label}
            >
              <Text style={[styles.label, { color: tcMap[variant] }]}>{btn.label}</Text>
            </Pressable>
          </View>
        )
      })}
