import { THUMB_TOTAL } from '@/src/components/atoms/CameraThumb'
...
export default function CameraScreen() {
  ...
  const onPressIn = useCallback(() => { shutterScale.value = withTiming(0.88, { duration: 70, easing: Easing.out(Easing.quad) }) }, [shutterScale])
  const onPressOut = useCallback(() => { shutterScale.value = withTiming(1, { duration: 140, easing: Easing.out(Easing.back(1.5)) }) }, [shutterScale])

  if (!hasPermission) return (
    <View style={styles.gate}>
      <Stack.Screen options={{ headerShown: false }} />
      <Text style={styles.gateTitle}>Camera Access Required</Text>
      <Text style={styles.gateBody}>FeralSpotter needs camera access to capture cat observations.</Text>
      <Pressable onPress={requestPermission} style={styles.gatePrimary} accessibilityRole="button">
        <Text style={styles.gatePrimaryText}>Allow Camera</Text>
      </Pressable>
      <Pressable onPress={() => Linking.openSettings()} style={styles.gateSecondary} accessibilityRole="button">
  ...
  if (!device) return (
    <View style={styles.gate}>
      ...
      <Pressable onPress={handleClose} style={styles.gatePrimary} accessibilityRole="button">
  ...
  const FlashIcon = flashMode === 'on' ? <Zap size={22} color={theme.colors.warning} /> : <ZapOff size={22} color={theme.colors.text} />
  ...
  return (
    <View style={styles.root}>
      ...
      <View style={styles.topBar}>
        {hasPhotos
          ? <Pressable onPress={handleDone} style={styles.pill} accessibilityRole="button">
            <Text style={styles.pillText}>Done ({capturedPhotos.length})</Text>
          </Pressable>
          : <Pressable onPress={handleClose} style={styles.iconBtn} accessibilityRole="button">
            <X size={24} color={theme.colors.text} />
          </Pressable>
        }
        <Pressable onPress={cycleFlash} style={styles.iconBtn} accessibilityRole="button">
          {FlashIcon}
      ...
      <View style={styles.bottomBar}>
        ...
        <View style={styles.shutterRow}>
          <Pressable onPress={flipCamera} style={styles.sideBtn} accessibilityRole="button">
            <SwitchCamera size={30} color={theme.colors.text} />
          </Pressable>
          <Animated.View style={shutterStyle}>
            <Pressable onPress={handleTakePhoto} onPressIn={onPressIn} onPressOut={onPressOut}
              disabled={isTakingPhoto} style={[styles.shutter, isTakingPhoto && styles.shutterBusy]}
