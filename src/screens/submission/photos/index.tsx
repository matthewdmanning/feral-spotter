    ...
    <>
      <ScrollView style={styles.scroll} keyboardShouldPersistTaps="handled"
        ...
        <View style={styles.inner}>
          ...
          <View style={styles.sourceRow}>
            {[{ label: 'Camera', icon: <Camera size={18} color={theme.colors.text} />, onPress: capturePhoto },
            { label: 'Library', icon: <ImagePlus size={18} color={theme.colors.text} />, onPress: pickFromLibrary }
            ].map(({ label, icon, onPress }) => (
              <Pressable key={label} onPress={onPress} style={styles.sourceBtn} accessibilityRole="button">
                {icon}<Text style={styles.sourceBtnText}>{label}</Text>
              </Pressable>
            ))}
          </View>
          <Text style={styles.status}>
            {checkedCount > 0 ? `${checkedCount} of ${sessionPhotos.length} included`
              : sessionPhotos.length > 0 ? 'Tap a photo to preview and include'
      ...
      <View style={styles.footer}>
        <Pressable onPress={handleDone} style={styles.doneBtn} accessibilityRole="button">
          <Text style={styles.doneBtnText}>{checkedCount > 0 ? `Done (${checkedCount})` : 'Done'}</Text>
        </Pressable>
      </View>

      {previewPhoto && (
        <PhotoPreviewModal photo={previewPhoto} isChecked={checked[previewPhoto.local_id] ?? false}
          onClose={() => setPreviewPhoto(null)} onToggle={() => toggleChecked(previewPhoto.local_id)} />
      )}
    </>
  )
}
