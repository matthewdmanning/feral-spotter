    ...
    <View style={styles.root}>
      ...
      <View style={styles.card}>
        ...
        <View style={styles.fieldGroup}>
          ...
          <TextInput placeholder="Enter address" placeholderTextColor={theme.colors.muted}
            value={address} onChangeText={handleAddressChange} onBlur={handleAddressBlur}
            style={styles.input} />
        </View>
        <View style={styles.footerGroup}>
          {saveIndicatorText !== '' && <Text style={styles.saveIndicator}>{saveIndicatorText}</Text>}
          <Pressable onPress={handleContinue} disabled={!locationType || !timeType} style={styles.continueBtn}>
      ...
      {cats.length > 0 && (
        <View style={styles.catList}>
          ...
          {cats.map((cat) => (
            <Pressable key={cat.local_id}
              onPress={() => router.push({ pathname: '/submission/cats', params: { edit: cat.local_id } })}
              style={styles.catRow}>
              <Text style={styles.catRowText}>
                {cat.age.charAt(0).toUpperCase() + cat.age.slice(1)} · {cat.pattern} · {cat.hair_length} hair
              </Text>
            </Pressable>
          ))}
        </View>
